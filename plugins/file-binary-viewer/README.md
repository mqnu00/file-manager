...
- **整页保存**：有修改的页点"保存本页"一次性提交（`write-range` 定位写入）；保存后以新内容刷新视图
...
- 读取：`GET /api/file-viewer/bytes?path=&offset=&length=`（base64，单次 ≤512KB）
- 保存：`POST /api/file-viewer/write-range`（`{path, offset, data(base64)}` 定位写入）
