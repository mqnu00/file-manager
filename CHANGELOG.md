...
- **CPU 信息**：型号、核心数、频率、实时使用率（进度条）
- **内存信息**：总量、已用、可用、使用率（进度条）
- **硬盘信息**：存储路径、总量、已用、可用、使用率（进度条）
- **Node.js 信息**：版本号、进程 ID

#### 后端新增接口

- 新增 `GET /api/system` 系统信息接口，返回 OS、CPU、内存、硬盘、Node.js 信息

#### UI 改进

- 工具栏新增"系统信息"按钮（Monitor 图标），位于主题切换和设置按钮之间
- 系统信息页面采用卡片式布局，自动适配赛博/亮色双主题
- 使用率数据通过进度条可视化展示，颜色根据使用率动态变化（绿/黄/红）
