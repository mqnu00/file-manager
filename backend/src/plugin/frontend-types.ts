 ...
 * @keep-in-sync frontend/src/pluginActions.ts
...
// ==================== 批量操作注册表类型（window.__fm_bulk_actions） ====================

/** 操作按钮可见性判定入参（与 frontend/src/pluginActions.ts 同步） */
...
/** 操作点按时的上下文（由主应用在点击时刻快照捕获；与 frontend/src/pluginActions.ts 同步） */
...
/** 插件注册的批量操作（与 frontend/src/pluginActions.ts 同步） */
...
/** 批量操作注册表 API（window.__fm_bulk_actions；与 frontend/src/pluginActions.ts 同步） */
export interface BulkActionsApi {
  /** 注册操作；同 id 覆盖替换。返回注销函数 */
  register(action: BulkAction): void
  /** 按 id 移除已注册操作（插件 teardown 用）；不存在则为 no-op */
  unregister(id: string): void
  /** 当前全部操作（注册顺序） */
  list(): BulkAction[]
  /** 注册表变化订阅（插件注册/覆盖后触发，主应用据此重渲染），返回取消订阅函数 */
  subscribe(fn: () => void): () => void
}

// ==================== 页面导航注册表类型（window.__fm_nav_actions） ====================

/** 页面导航注册表 API（window.__fm_nav_actions；与 frontend/src/pluginNav.ts 同步） */
